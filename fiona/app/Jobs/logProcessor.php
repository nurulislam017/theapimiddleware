...
class logProcessor implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if (isset($this->log->logger_key)) {
            ...
            // Check if the URL exists for the given host
    ...
    where 	
        ...
        and clusters.host = ?", [(parse_url($this->log->url, PHP_URL_PATH) ?? '/'), $this->log->host]);
        ...
        if (count($check_main) > 0) {
            ...
            foreach ($check_main as $checks) {
                if ($checks->rule == 'logging_http') $logging_http = $checks->value;
                if ($checks->rule == 'logging_gdpr') $logging_gdpr = $checks->value;
                if ($checks->rule == 'redact_auth') $redact_auth = $checks->value;
                if ($checks->rule == 'required_auth') $required_auth = $checks->value;
                if ($checks->rule == 'encryption') $encryption = $checks->value;
                if ($checks->rule == 'global_rpm') $global_rpm = $checks->value;
                if ($checks->rule == 'anomally_detection') $anomaly_detection = $checks->value;
                if ($checks->rule == 'honey_pots') $honey_pots = $checks->value;
                if ($checks->rule == 'pii_dlp') $pii_dlp = $checks->value;
                if ($checks->rule == 'user_rpm') $user_rpm = $checks->value;
                if ($checks->rule == 'access_type') $access_type = $checks->value;
                if ($checks->rule == 'users') $users = json_decode($checks->value, true);
            }
            ...
            if ($redact_auth == 1) {
                ...
                $this->log->request_headers = json_encode($req_headers);
                ...
                $this->log->prams = json_encode(['s'=>'s']);
            }
            if ($logging_http == 0) {
                ...
                $log->prams = ['Cluster Rule' => 'HTTP Logging Disabled'];
            }
        }
