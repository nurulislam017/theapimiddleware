<x-app-layout :data=[$domain,$start_time,$end_time]>
    <x-slot name="page">{{ __('API Clusters') }}</x-slot>
    <x-slot name='crumb'>
        <li>
        ...
        <li aria-current="page">
            <div class="flex items-center">
                ...
                <span class="ms-1 text-sm font-medium text-zinc-500">Clusters</span>
            </div>
        </li>
    </x-slot>
            ...
            <table class="w-full text-sm" id="default-table">
                <thead>
                    <tr class="border-b border-zinc-100">
                        ...
                        <th class="px-5 py-2.5 text-left text-xs font-medium text-zinc-400 uppercase tracking-wide">Policy</th>
                        <th class="px-5 py-2.5 text-right text-xs font-medium text-zinc-400 uppercase tracking-wide">APIs</th>
                        <th class="px-5 py-2.5 text-center text-xs font-medium text-zinc-400 uppercase tracking-wide"></th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-zinc-50">
                    @foreach($clusters as $cluster)
                    @livewire('apis.cluster.view', ['cluster' => $cluster, 'domain' => $domain])
                    @endforeach
                </tbody>
            </table>
        </div>
        <div class="px-5 py-3 border-t border-zinc-100 flex justify-end">
            <a href="{{ route('api_cluster_edit', ['domain' => base64_encode($domain), 'cluster' => 'new', 'start_datetime' => request()->get('start_datetime'), 'end_datetime' => request()->get('end_datetime')]) }}"
