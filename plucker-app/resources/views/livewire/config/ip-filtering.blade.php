<div>
    <div class="bg-white border border-zinc-200 rounded-xl overflow-hidden">
        <div class="px-5 py-3 border-b border-zinc-100">
            <p class="text-xs text-zinc-500 uppercase tracking-wide font-medium">IP Filtering</p>
        </div>
        <div class="p-5">
            @if($update == 'true')
            <div class="flex items-center gap-2 p-3 mb-5 text-sm text-green-700 bg-green-50 border border-green-200 rounded-lg">
                ...
                <span><span class="font-medium">Policy saved.</span> IP filtering is now active.</span>
            </div>
            @endif
            ...
            <form wire:submit="save">
                ...
                <div class="mb-5">
                    <label class="block mb-1.5 text-xs font-medium text-zinc-500 uppercase tracking-wide">IP List <span class="normal-case">(one per line)</span></label>
                    <textarea rows="5" wire:model.fill='list'
                              ...
                              placeholder="192.168.1.1&#10;10.0.0.0/24" required>@php foreach($ips as $ip) { echo "$ip->ip\n"; } @endphp</textarea>
                </div>
